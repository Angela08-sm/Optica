      ...
      <div class="modal-body">
        <div class="modal__content">
          ...
          <div class="widget-social-icons mt-sm-5 mt-4">
            <h5 class="widget-title">Historial clínico</h5>
...
<br><br><br>
<a class="btooninsertar" href="../Agenda/InsertarAgenda.php"> + AGREGAR HISTORIAL CLÍNICO NUEVO</A>
<br><br><br>
<div class="cajatabla">
  <table class="table">
    <thead>
      <tr>
        <th>ID_HISTORIAL_CLINICO</th>
        <th>ID_PACIENTE </th>
        <th>ID_CONSULTA</th>
        <th>EXAMENES</th>
        <th>ACTUALIZAR</th>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (empty($_POST['TXTIDPACIENTE']) || empty($_POST['TXTIDCONSULTA']) || empty($_POST['EXAMENES'])) {
            echo "<script>alert('Todos los campos son obligatorios.')</script>";
        } else {
            $IDPA = $_POST['TXTIDPACIENTE'];
            $IDCON = $_POST['TXTIDCONSULTA']; 
            $EXAMEN = $_POST['EXAMENES'];  
      

   

            // Comprobar si el usuario ya tiene una agenda
            $query = mysqli_query($conexion, "SELECT COUNT(Id_historial_clinico) AS IDS FROM historial_clinico WHERE Id_paciente='$IDPA'");
            $fila = mysqli_fetch_assoc($query);
            $cuenta = $fila['IDS'] ?? 0;

            if ($cuenta > 0) {
                echo "<script>alert('El usuario ya tiene un historial clinico registrado.')</script>";
            } else {
    ...
    ?>
        <tr class="tablaa">
            ...
            <td><?php echo $HFIN; ?></td>
            <td><a href="Update.php?ACTUALIZAR=<?php echo $IDAGENDA; ?>"> <img style="width: 50px;height: 50px;" src="../assets/images/actualizar-flecha.png"> </a></td>
            ...
            <td><a href="../FACTURACION/FACTURA.PHP?FACTURAR=<?php echo $IDAGENDA; ?>"> <img style="width: 50px;height: 50px;" src="../assets/images/factura.png"></a></td>
        </tr>
    <?php } ?>
